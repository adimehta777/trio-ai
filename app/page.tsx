...
function loadKeys() { try { return JSON.parse(localStorage.getItem(STORAGE_KEY) || "{}"); } catch { return {}; } }
...
function ResponseCard({ model, response, loading, error }) {
  return (
...
function KeysModal({ keys, onSave, onClose }) {
...
export default function TrioAI() {
  ...
  const [responses, setResponses] = useState({ claude:null, gemini:null, gpt:null });
  const [loading, setLoading] = useState({ claude:false, gemini:false, gpt:false });
  const [errors, setErrors] = useState({ claude:null, gemini:null, gpt:null });
